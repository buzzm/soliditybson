...
// Apparently wherever MyContract.sol is found from solc, if you say 
//     import "./file.sol"
...
contract MyContract {
     ...
     * functions.  In general, best practice to externalize BSON state is to
     * have two functions: one internal that can take advantage of structs and
     * other functions, and an external one that is exposed to the public, e.g.
     *
     *    function makeState(any kinds of args) view internal
     *        returns (BsonUtils.BsonDocument memory doc)
     *
     *    function getState(simple args only) view public
	...
	// Remember: no realloc() in solidity.  We must set up the array of
	// "slots" in advance:
	...
	for(uint8 j = 0; j < numPeeps; j++) {
	    ...
	    peep[1] = BsonUtils.BsonMap('num', BsonUtils.createBsonInt32(int8(j*5))); // fun
     ...
     */
    function getStateFOO() public view returns (bytes memory) {
	...
	if(notify == true) {
	    BsonUtils.BsonDocument memory doc = makeStateFOODocument(4);
	    bytes memory info = BsonUtils.toBytes(doc);
	    emit Modified(info);
	}
    }
     ...
     *  Note how tedious it is on the Java side....
     */

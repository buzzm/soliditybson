...
/*
  Note:
  Among other things, v 0.8 has breaking changes where you cannot in one
  cast change more than bitsize and sign (and payable but we don't care about
  that here).  Thus, to de-complicate the code, we tend to use larger signed
  types like int64 especially when *anything* in the calc is negative.
*/
...
library BsonUtils {
    ...
    bytes1 private constant BSON_CODE_UNSET = 0x00;

    bytes1 private constant BSON_CODE_DOUBLE = 0x01;
    bytes1 private constant BSON_CODE_STRING = 0x02;
    bytes1 private constant BSON_CODE_DOCUMENT = 0x03;
    bytes1 private constant BSON_CODE_ARRAY = 0x04;
    bytes1 private constant BSON_CODE_BINARY = 0x05;
    bytes1 private constant BSON_CODE_DATETIME = 0x09;
    bytes1 private constant BSON_CODE_INT32 = 0x10;
    bytes1 private constant BSON_CODE_INT64 = 0x12;
    bytes1 private constant BSON_CODE_DEC128 = 0x13;

    bytes1 private constant BSON_CODE_UNKNOWN = 0xFF;


    // subtypes
    bytes1 private constant BSON_CODE_BINARY_GENERIC = 0x00; 
	...
	//    BsonValue memory vv;
	// sets all members to default value which for byte will be zero
	// so we are good.
	bytes1 typ;
	...
	if(ndigits != 0) {
	    ...
	    bv.v_str[14] = bytes1(uint8(64 - (2*shiftpts)));
	}
	...
	if(yr >= 1970) {
		...
		totdays += int64(int8(dpm[n]));
	    }

	    totdays += int64(int8(d-1));
	    ...
	    totms += (int64(int8(hr))  * MILLIS_PER_HOUR);
	    totms += (int64(int8(min)) * MILLIS_PER_MIN);
	    totms += (int64(int8(sec)) * MILLIS_PER_SEC);
	    totms += int64(int16(ms));
	    ...
	    // back up...
	    int64 accum_ms = int64((23 - int64(int8(hr))) * MILLIS_PER_HOUR);
	    accum_ms += int64((59 - int64(int8(min))) * MILLIS_PER_MIN);
	    accum_ms += int64((59 - int64(int8(sec))) * MILLIS_PER_SEC);
	    accum_ms += int64(1000 - int64(int16(ms)));
	...
	int64 inMillis = floorMod(millis, 1000);
	...
	DateTime memory dtx;  
        ...
        dtx.v_month = uint8( int8( (marchMonth0 + 2) % 12 + 1  ));
        dtx.v_day   = uint8( int8( doyEst - (marchMonth0 * 306 + 5) / 10 + 1) );

        yearEst += marchMonth0 / 10;
	dtx.v_year  = uint16(int16(yearEst));


	int64 tmp_64_holder; // to cut down on the mess of casting...
	
	int64 millisOfDay = (secsOfDay * MILLIS_PER_SEC) + inMillis;

	tmp_64_holder = millisOfDay / MILLIS_PER_HOUR;
	dtx.v_hour = uint8(int8(tmp_64_holder));
	millisOfDay -= (tmp_64_holder * MILLIS_PER_HOUR);

	tmp_64_holder = millisOfDay / MILLIS_PER_MIN;
	dtx.v_minute = uint8(int8(tmp_64_holder));
        millisOfDay -= (tmp_64_holder * MILLIS_PER_MIN);

	tmp_64_holder = millisOfDay / MILLIS_PER_SEC;
	dtx.v_second = uint8(int8(tmp_64_holder));
        millisOfDay -= (tmp_64_holder * MILLIS_PER_SEC);

	dtx.v_ms = uint16(int16(millisOfDay));
	...
	// More or less in order of frequency of appearance in data:
	if(BSON_CODE_STRING == op) {
	    int32 slen = LEbytesToInt32(bsonbytes, idx);
	    idx += 4;
	    val = createBsonString(extractString(bsonbytes, idx));
	    idx = idx + uint32(slen);

	} else if(BSON_CODE_INT32 == op) {
    ...
    */
    ...
    function processInt64(bool doWrite, bytes memory buf, uint32 idx, int64 x, bytes1 code, bytes memory name) pure private returns (uint32) {
        uint32 nlen = uint32(name.length);
